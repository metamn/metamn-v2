// Based on http://codepen.io/Gthibaud/pen/pyeNKj
...
function particle(x, y, bubbleSize) {
  this.radius = 1.1;
  ...
  this.update = function(heading) {
    ...
    ctx.arc(this.x, this.y, this.radius / this.bubbleSize, Math.PI * 2, false);
    ctx.fill();
    ctx.closePath();
...
// The basic class drawing the text on the canvas
function shape(x, y, text, ctx, fontSize, bubbleSize) {
  this.x = x;
  this.y = y;
  this.size = fontSize;
...
shape.prototype.getValue = function() {
  ...
  for (var y = 0; y < H; y += gridY) {
    for (var x = 0; x < W; x += gridX) {
      if (buffer32[y * W + x]) {
        this.placement.push(new particle(x, y, bubbleSize));
      }
    }
  }
...
// The main function
var textParticle = function(elementID) {
